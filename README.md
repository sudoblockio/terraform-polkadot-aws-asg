...
| user_data | github.com/geometry-labs/terraform-polkadot-user-data.git |  |
...
|------|-------------|------|---------|:--------:|
| additional\_build\_security\_group\_ids | Additional security groups to use to build image. | `list(string)` | <pre>[<br>  ""<br>]</pre> | no |
| ami\_id | AMI ID to use in autoscaling group. Blank to build from packer. | `string` | `""` | no |
| base\_path | Base path for Substrate | `string` | `""` | no |
| build\_security\_group\_id | The security group to use to build image. | `string` | `""` | no |
...
| consul\_tls\_ca\_filename | Filename of Consul CA | `string` | `""` | no |
| consul\_tls\_source\_dir | Path to TLS files | `string` | `""` | no |
...
| envoy\_enabled | Configure Envoy proxy for Consul Connect | `bool` | `false` | no |
...
| node\_exporter\_enabled | Bool to use when node exporter is enabled | `bool` | `false` | no |
...
| polkadot\_additional\_common\_flags | Additonal common flags for substrate | `string` | `""` | no |
| polkadot\_additional\_validator\_flags | Additonal common flags for validator | `string` | `""` | no |
...
| source\_of\_truth\_enabled | Bool to use when SOT is enabled | `bool` | `false` | no |
